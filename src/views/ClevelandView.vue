...
const clevelandData = ref<clevelandObject[]>([])
...
async function getClevelandData(term: string) {
  searchController?.abort()
  searchController = new AbortController()
  loading.value = true

  try {
    const data = await searchClevelandArtworks(term, 0, { signal: searchController.signal })
    clevelandData.value = data.artworks
    nextStart.value = data.pagination.hasMore ? data.pagination.limit : null
  } catch (error) {
...
async function loadMoreClevelandData() {
  if (nextStart.value === null) return

  loadingNext.value = true

  try {
    const data = await searchClevelandArtworks(searchTerm.value, nextStart.value)
    clevelandData.value = [...clevelandData.value, ...data.artworks]
    nextStart.value = data.pagination.hasMore ? nextStart.value + data.pagination.limit : null
  } catch (error) {
    console.error(error)
  } finally {
...
function saveArtwork(artwork: clevelandObject) {
  if (!userStore.user) {
    router.push('/login')
    return
  }
  ...
  const isSaved =
    artworkStore.cleveland.includes(artwork.externalId) ||
    savedArtworks.value.includes(artwork.externalId)
  isSubmitting.value = true

  const request = isSaved
    ? deleteArtworkBySource('cleveland', artwork.externalId)
    : saveArtworkRequest({ source: 'cleveland', artwork })

  request
    .then(() => {
      if (isSaved) {
        savedArtworks.value = savedArtworks.value.filter((id) => id !== artwork.externalId)
        artworkStore.removeArtwork('cleveland', artwork.externalId)
      } else {
        savedArtworks.value.push(artwork.externalId)
...
function mapArtworks() {
  ...
  fetchSavedArtworkMap()
    .then((savedMap) => {
      savedArtworks.value = savedMap.cleveland
...
function openModal(artwork: clevelandObject) {
  ...
  imgUrl.value = artwork.image
  imgAlt.value = artwork.title
  modalOpen.value = true
}

function getHoverItems(artwork: clevelandObject) {
  return [artwork.title, artwork.artist, artwork.date, artwork.medium, artwork.department].filter(
    Boolean
  ) as string[]
}
      ...
      <div v-show="clevelandData.length > 0">
        ...
        >
          <template #default="{ item }">
              ...
              <HeartIcon
                ...
                :class="{
                  'fill-red-600 text-red-600':
                    artworkStore.cleveland.includes(item.externalId) ||
                    savedArtworks.includes(item.externalId),
                  'text-white':
                    !artworkStore.cleveland.includes(item.externalId) &&
                    !savedArtworks.includes(item.externalId)
                }"
            ...
            <ArtworkHoverInfo source-label="Cleveland Museum of Art" :items="getHoverItems(item)" />
          </template>
        </TumblrMasonryGallery>
